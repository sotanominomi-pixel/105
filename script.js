...
// (続き)
        ...
        try {
            // 1. iOSでバックグラウンド維持するためにビデオ要素の属性を補強
            video.muted = true;
            video.playsInline = true;
            video.disablePictureInPicture = false;

            // 2. Canvasからストリームを生成（30fpsで滑らかに）
            const stream = pipCanvas.captureStream(30);
            video.srcObject = stream;

            // 3. ビデオを再生（iOSでは再生されていないとPiPにできない）
            await video.play();

            // 4. PiPをリクエスト
            await video.requestPictureInPicture();

            // 5. PiPが成功したら、Web上のフローティング要素は隠す（二重表示防止）
            const fc = document.getElementById('floating-clock');
            if (fc) fc.style.display = 'none';

        } catch (error) {
            console.error('iOS PiP Error:', error);
            alert('PiPの起動に失敗しました。画面を一度タップしてから再度お試しください。');
        }
    }
};

// 6. 【iOS維持ロジック】アプリが背面へ回ってもCanvasを更新し続ける
// requestAnimationFrameは背面で止まるため、setIntervalを併用します
setInterval(() => {
    if (document.pictureInPictureElement) {
        // バックグラウンドでもCanvas(pipCtx)を強制的に再描画して
        // ビデオストリームが「静止画」と判定されて切断されるのを防ぐ
        updateClock();
    }
}, 1000);

// 7. モバイルモード時のボタンを「ホーム画面対応PiP」にアップグレード
const _finalRenderClockMode = renderClockMode;
renderClockMode = function() {
    _finalRenderClockMode();
    const pipContainer = document.querySelector('.pip-button-container');
    if (pipContainer && deviceMode === 'mobile') {
        pipContainer.innerHTML = `
            <button onclick="togglePiP()" class="action-button pip-btn" style="background-color: #f39c12; width: 85%; margin: 10px auto; display: block; border-radius: 25px;">
                ${currentLang === 'ja' ? 'ホーム画面に時計を出す' : 'Show Clock on Home'}
            </button>
        `;
    }
};
