...
/* --- 【追加改善】スマホ（iOS/Android）向けPiP強制トリガーアドオン --- */

(function() {
    // 既存のvideo要素をスマホが「動画」として認識しやすいように属性を追加
    const setupMobileVideo = () => {
        const video = document.getElementById('pip-video');
        if (!video) return;
        
        // スマホでPiPを許可するための必須属性をJSから注入
        video.setAttribute('webkit-playsinline', 'true');
        video.setAttribute('playsinline', 'true');
        video.muted = true; // スマホはミュート必須
        video.disablePictureInPicture = false;
    };

    // スマホSafari等でのPiP起動を補助する関数
    const triggerMobilePiP = async () => {
        ...
        try {
            ...
            // 2. スマホでは「再生開始」と「PiPリクエスト」をほぼ同時に行う必要がある
            await video.play();

            if (video.webkitSupportsPresentationMode && typeof video.webkitSetPresentationMode === "function") {
                // iOS Safari 独自のPiP起動方法
                video.webkitSetPresentationMode("picture-in-picture");
            } else if (document.pictureInPictureEnabled || video.requestPictureInPicture) {
                // 標準API
                await video.requestPictureInPicture();
            } else {
                throw new Error("Your browser does not support PiP.");
            }
        } catch (err) {
            console.error("Mobile PiP Error:", err);
            // 失敗した場合は既存の「フローティング時計（Mobileモード用）」を表示
            if (typeof toggleFloatingClock === 'function') toggleFloatingClock();
        }
    };

    // 既存のボタンクリック時にスマホ用トリガーも走るようにイベントを「追加」
    window.addEventListener('DOMContentLoaded', () => {
        setupMobileVideo();
        const pipBtn = document.getElementById('pip-start-btn');
        if (pipBtn) {
            pipBtn.addEventListener('click', (e) => {
                // 標準のtogglePiPが動かない場合に備えてスマホ用を走らせる
                triggerMobilePiP();
            });
        }
    });
