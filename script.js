...
/* --- 【iOS完全対応】ホーム画面でも消えないPiP維持アドオン --- */
(function() {
    const video = document.getElementById('pip-video');
    
    // 既存の togglePiP を iOS 用に強化して上書き
    const iosEnhancedTogglePiP = async function() {
        try {
            if (document.pictureInPictureElement) {
                await document.exitPictureInPicture();
            } else {
                // 1. ストリームの生成（フレームレートを安定させる）
                const stream = pipCanvas.captureStream(30); 
                video.srcObject = stream;

                // 2. iOSでバックグラウンド再生を維持するための「おまじない」
                video.play();

                // 3. ユーザー操作（クリック）の直後である必要があるため、ここでリクエスト
                await video.requestPictureInPicture();

                // 4. バックグラウンド移行時の切断防止対策
                // iOSはCanvasの更新が止まるとPiPが閉じるため、
                // 画面が非表示になってもCanvasの描画を強制継続させる。
                console.log("iOS PiP Mode Activated");
            }
        } catch (error) {
            console.error('PiP失敗:', error);
            alert('PiPの起動に失敗しました。');
    ...
    // グローバルな関数を差し替え（元のコードのロジックを尊重しつつ上書き）
    window.togglePiP = iosEnhancedTogglePiP;

    // モバイルモード時のボタンイベントを再バインド
    const originalRenderClock = renderClockMode;
    renderClockMode = function() {
        originalRenderClock();
        const pipBtn = document.getElementById('pip-start-btn');
        if (pipBtn) {
            pipBtn.onclick = iosEnhancedTogglePiP;
        }
        // モバイルモード用ボタンも上書き
        const mobilePipBtn = document.querySelector('button[onclick="toggleFloatingClock()"]');
        if (mobilePipBtn && deviceMode === 'mobile') {
            // モバイル時はフローティング要素ではなく、本物のPiPを優先させる改善
            mobilePipBtn.onclick = iosEnhancedTogglePiP;
            mobilePipBtn.textContent = currentLang === 'ja' ? 'PiP時計を起動' : 'Start PiP Clock';
        }
    };

    // 5. 【最重要】iOSのバックグラウンド描画維持ロジック
    // requestAnimationFrameはバックグラウンドで止まるため、setIntervalを併用して
    // 非表示状態でもCanvas(pipCtx)を更新し続けることでストリームを維持します。
    setInterval(() => {
        if (document.pictureInPictureElement) {
            // updateClockを強制呼び出ししてCanvasを更新し続ける
            updateClock();
        }
    }, 1000); // 1秒ごとに描画を更新して「生きてる」ことをOSに伝える
})();
